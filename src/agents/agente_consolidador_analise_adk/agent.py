...
try:
    ...
    print(f"PROJECT_ROOT ({PROJECT_ROOT}) foi adicionado/confirmado no sys.path para agent.py (AgenteConsolidadorAnalise).")
except NameError:
    PROJECT_ROOT = Path(os.getcwd())
    if str(PROJECT_ROOT) not in sys.path:
        sys.path.insert(0, str(PROJECT_ROOT))
    print(f"AVISO (agent.py AgenteConsolidadorAnalise): __file__ não definido. Usando PROJECT_ROOT como: {PROJECT_ROOT}")
...
try:
    ...
    print("Módulos do projeto e ADK importados com sucesso para AgenteConsolidadorDeAnalise_ADK.")
