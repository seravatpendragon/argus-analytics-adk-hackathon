...
collect_eia_tool = FunctionTool(func=collect_and_store_eia_indicators)

AgenteColetorEIA_ADK = Agent(
    name="agente_coletor_eia_adk_v1",
    model=MODELO_LLM_AGENTE,
    description="Agente responsável por coletar séries de dados do EIA.",
    instruction=agente_prompt.PROMPT,
    tools=[collect_eia_tool],
)

settings.logger.info(f"Agente '{AgenteColetorEIA_ADK.name}' carregado com o modelo '{MODELO_LLM_AGENTE}'.")
...
if __name__ == '__main__':
    ...
    async def run_standalone_test():
        ...
        prompt_text = "Por favor, inicie a coleta de dados do EIA."
