...
collect_fgv_tool = FunctionTool(func=collect_and_store_fgv_indicators)

AgenteColetorFGV_ADK = Agent(
    name="agente_coletor_fgv_adk_v1",
    model=MODELO_LLM_AGENTE,
    description="Agente responsável por coletar séries de dados do FGV/SIDRA.",
    instruction=agente_prompt.PROMPT,
    tools=[collect_fgv_tool],
)

settings.logger.info(f"Agente '{AgenteColetorFGV_ADK.name}' carregado com o modelo '{MODELO_LLM_AGENTE}'.")
...
if __name__ == '__main__':
    ...
    async def run_standalone_test():
        ...
        prompt_text = "Por favor, inicie a coleta de dados do FGV."
