...
try:
    ...
    if not hasattr(settings, 'logger'):
        ...
        settings.logger.info(f"PROJECT_ROOT ({PROJECT_ROOT}) foi adicionado/confirmado no sys.path para agent.py (SubAgenteSentimento).")
        settings.logger.info("Módulos do projeto e ADK importados com sucesso para SubAgenteSentimento_ADK.")
